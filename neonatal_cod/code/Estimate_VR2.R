...
for (i in lams){
  nam <- paste("vr2",i,sep="_")
  assign(nam,f.e1(STUD=studies2, DEAT=deaths2, LAMB=i, SAVX=F, SADE=F, NAME=paste("vr2",i,sep="_")))
  save(list=paste("vr2",i,sep="_"), file = paste0("../results/VR2.RData"))
  do.call(rm, list(paste("vr2",i, sep="_")))
}
...
#for (i in lams) load(paste0("../results/VR2.RData"))
#vr2nams <- ls()[grep("vr2_", ls())]
#mod.names <- rownames(get(vr2nams[1])[["output"]][["BUGSoutput"]][["summary"]])
#
#for (v in vr2nams) {
